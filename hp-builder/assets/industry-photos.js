...
/* 置き場所。写真は、道具のすぐ隣に置いてある。
   ...
   最後の頼りは、受け口（PUBLISH.endpoint）のあるところ。写真は道具と
   同じところに置いてあり、公開されたページの置き場とは別のことがある
   （別のドメインへ移したとき）。公開の入れ物を頼りにすると、そこには
   写真が無い。 */
const photoBases = () => {
  const out = [];
  ...
  } catch (e) { /* 開いた場所が読めない。下の受け口に頼る */ }
  try {
    if (typeof PUBLISH === 'object' && PUBLISH.endpoint) {
      add(new URL('/art/', PUBLISH.endpoint).href);
    }
  } catch (e) { /* 受け口の形が読めない */ }
  return out;
};
