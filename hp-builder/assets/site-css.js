...
/* ---------- あとから足すパーツ ----------
   中身の下に、足したパーツを縦に積む。中身が中央ぞろえの段では中央、
   左ぞろえのヒーローでは左に寄せる（テキストは1つずつ向きを選べる）。 */
.parts{display:flex;flex-direction:column;align-items:center;
  gap:clamp(12px,1.6vw,20px);margin-top:clamp(22px,2.6vw,36px)}
.hero.left .parts,.hero.split .parts,.hero.pack .parts,.sec-head.left ~ * .parts{align-items:flex-start}
.parts .btn-row{margin-top:0}
.parts .pt-btns{justify-content:inherit}
.pt-txt{margin:0;max-width:36em}
.pt-a-left{text-align:left;align-self:flex-start}
.pt-a-center{text-align:center}
.pt-a-right{text-align:right;align-self:flex-end}
.pt-big{font-size:clamp(20px,1.9vw,30px);font-weight:800;line-height:1.42;
  letter-spacing:-.01em;color:var(--c-text)}
.pt-normal{font-size:clamp(15px,.6vw + 13px,17px);line-height:1.85;color:var(--c-muted)}
.pt-small{font-size:13px;line-height:1.7;color:var(--c-muted);letter-spacing:.02em}
.pt-tag{display:inline-flex;align-items:center;padding:6px 15px;border-radius:999px;
  font-size:12.5px;font-weight:700;letter-spacing:.06em;
  background:color-mix(in srgb,var(--c-primary) 12%,transparent);color:var(--c-primary)}
.pt-line{display:block;width:clamp(40px,5vw,64px);height:2px;border-radius:2px;
  background:color-mix(in srgb,currentColor 24%,transparent)}
@supports not (color:color-mix(in srgb,red,blue)){
  .pt-tag{background:rgba(128,128,128,.14);color:var(--c-primary)}
  .pt-line{background:rgba(128,128,128,.34)}
}
/* 写真いっぱい・濃い地のヒーローでは、地が暗いので明るい字にする */
.hero.cover .pt-normal,.hero.cover .pt-small,
.hero.bg-dark .pt-normal,.hero.bg-dark .pt-small,
.hero.bg-primary .pt-normal,.hero.bg-primary .pt-small{color:rgba(255,255,255,.9)}
.hero.cover .pt-big,.hero.bg-dark .pt-big,.hero.bg-primary .pt-big{color:#fff}
...
/* ==========================================================
   シェイプ・ヒーロー（丸つなぎ・ななめ帯・波の棒）

   土台（ぼかし）も影も付けない。抜いた形の写真を、そのまま
   大きく、画面の右端まで広げる。文字は左に、控えめに置く。
   ========================================================== */
.hero.split:is(.shp-dots,.shp-bars,.shp-wavebar){overflow:hidden}

/* 右側だけ枠の制限を外し、シェイプを画面の端まで届かせる。
   左は、ふつうの本文が始まる位置にそろえる。 */
.hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .wrap{
  max-width:none;
  padding-left:max(24px, calc((100vw - var(--max)) / 2 + 24px));
  padding-right:0;
}
.hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .hero-in{
  grid-template-columns:clamp(240px,30vw,380px) 1fr;
  gap:clamp(24px,4vw,72px);
  align-items:stretch;
  min-height:min(88svh,880px);
}
.hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .hero-in > div:first-child{
  align-self:center;
}
/* シェイプ単体：土台も影も無し。枠いっぱいに大きく。 */
.hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .hero-media{
  background:none;box-shadow:none;filter:none;transform:none;
  aspect-ratio:auto;height:100%;width:100%;
}
/* 抜き型には、まわりに透明の余白がある（形ごとに幅が違う）。
   そのままだと形が枠の中で小さく収まり、画面の端まで届かない。
   形ごとに抜き型を拡大して、余白を枠の外へ追い出し、中身を枠いっぱいに
   広げる（上下は少しはみ出して切れる＝大きく、端まで、の見え方）。
   拡大率は、各形の透明余白（左右）から決めている。 */
.hero.split.shp-dots .hero-media{
  -webkit-mask-size:180% 180%;mask-size:180% 180%;
  -webkit-mask-position:center;mask-position:center;
}
.hero.split.shp-bars .hero-media{
  -webkit-mask-size:134% 134%;mask-size:134% 134%;
  -webkit-mask-position:center;mask-position:center;
}
.hero.split.shp-wavebar .hero-media{
  -webkit-mask-size:114% 114%;mask-size:114% 114%;
  -webkit-mask-position:center;mask-position:center;
}

/* スマホ：縦に積む。シェイプは画面の横いっぱいに大きく、文字の下に。 */
@media(max-width:760px){
  .hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .wrap{
  ...
  /* 文字は左右に余白をつけて読みやすく（枠の padding を外したぶん） */
  .hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .hero-in > div:first-child{
    padding:0 24px;
  }
  .hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .hero-media{
    height:auto;aspect-ratio:1/1;width:100%;margin-top:clamp(20px,4vh,36px);
  }
}
/* ==========================================================
   単色に浮くシェイプ（emboss）

   土台とシェイプを同じ色にして、影だけで形の輪郭を伝える。
   同じ色のまま「そこだけ持ち上がっている」ように見せる型。
   文字はシェイプの上に乗せる（シェイプが中身を包む板になる）。

   影は2枚。近くにくっきりした濃い影（立体感の主役）と、
   遠くにやわらかい影（地に置いてある落ち着き）。どちらも土台と
   同じ色の濃いほうを使うので、色が濁らない。
   ========================================================== */
.hero.emboss{
  --emb-fill:var(--c-surface);
  --emb-hi:color-mix(in srgb,var(--emb-fill) 52%,#fff);
  --emb-sh:color-mix(in srgb,var(--emb-fill) 58%,#000);
  --emb-sh2:color-mix(in srgb,var(--emb-fill) 72%,#000);
  background:var(--emb-fill);
}
.hero.emboss.bg-primary{--emb-fill:var(--c-primary)}
.hero.emboss.bg-dark{--emb-fill:var(--c-dark)}
.hero.emboss .hero-in{min-height:min(80svh,780px);display:grid;place-items:center;
  padding:clamp(40px,7vw,110px) 0}
.emb-stage{display:grid;place-items:center;width:100%}
.emb-panel{
  position:relative;text-align:center;
  max-width:min(92vw,600px);
  padding:clamp(40px,6vw,84px) clamp(28px,5vw,66px);
  background:var(--emb-fill);
  /* 上の細い明るい縁＋近くのくっきり影＋遠くのやわらかい影 */
  box-shadow:
    inset 0 1px 0 var(--emb-hi),
    14px 18px 5px -5px var(--emb-sh),
    32px 42px 40px -18px var(--emb-sh2);
}
.emb-squircle{border-radius:clamp(40px,6vw,82px)}
.emb-circle{aspect-ratio:1/1;display:grid;place-items:center;border-radius:50%;
  width:min(92vw,560px);max-width:min(92vw,560px);padding:clamp(40px,7vw,84px)}
.emb-arch{border-radius:clamp(130px,22vw,290px) clamp(130px,22vw,290px) clamp(36px,5vw,70px) clamp(36px,5vw,70px)}
.emb-blob{border-radius:46% 54% 48% 52%/56% 46% 54% 44%}
/* シェイプの中に収まる大きさに抑える。土台いっぱいではなく、
   板の上に載る一言なので、通常のヒーローより一段小さくする。 */
.hero.emboss .hero-title{font-size:clamp(24px,2.6vw,42px);line-height:1.3}
.hero.emboss .hero-text{margin-inline:auto;font-size:clamp(14.5px,1vw,16.5px);max-width:26em}
.hero.emboss .eyebrow{margin-bottom:14px}
.hero.emboss .btn-row{justify-content:center;margin-top:clamp(22px,2.4vw,30px)}
.hero.emboss .parts{align-items:center}
@media(max-width:560px){
  /* 細い画面では、横いっぱいのボタンが円の左右カーブからはみ出す。
     円だけ角丸の板に切り替えて、中身が必ず収まるようにする。
     角丸・アーチ・まるみは、下辺がまっすぐなのでそのままで収まる。 */
  .emb-circle{aspect-ratio:auto;width:100%;max-width:100%;
    border-radius:clamp(38px,10vw,56px);
    padding:clamp(36px,8vw,52px) clamp(24px,6vw,38px)}
}
@supports not (color:color-mix(in srgb,red,blue)){
  .hero.emboss{--emb-fill:#f2f3f5;--emb-hi:rgba(255,255,255,.6);
    --emb-sh:rgba(15,23,42,.18);--emb-sh2:rgba(15,23,42,.12)}
}
