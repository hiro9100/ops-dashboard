...
/* ==========================================================
   ...
   土台（ぼかし）も影も付けない。抜いた形の写真だけを、大きく。
   ただし、画面からはみ出したり、左の文字に被って角がスパッと
   切れたりしないよう、枠の中にきちんと収める。上下も切らない。
   ========================================================== */
...
/* シェイプ単体：土台も影も無し。写真側を大きく。
   幅は枠の中に収める（画面の端やテキストに被らせない）。 */
...
/* 抜き型には、形ごとに違う透明の余白が左右にある（丸つなぎは2割ずつ、
   ななめ帯は1割、波の棒は少し）。そのままだと形が枠の中で小さく収まる。
   横だけ、その余白ぶんを詰めて枠幅いっぱいに広げる。縦はそのまま
   （＝上下を切らない）。枠の縦横比は、形の中身の比に合わせてゆがませない。 */
.hero.split.shp-dots .hero-media{aspect-ratio:1/1;-webkit-mask-size:177% 100%;mask-size:177% 100%}
.hero.split.shp-bars .hero-media{aspect-ratio:7/5;-webkit-mask-size:131% 100%;mask-size:131% 100%}
.hero.split.shp-wavebar .hero-media{aspect-ratio:8/5;-webkit-mask-size:110% 100%;mask-size:110% 100%}

/* スマホ：縦に積む。シェイプは文字の下に、枠いっぱいに（はみ出さない）。 */
@media(max-width:760px){
  .hero.split:is(.shp-dots,.shp-bars,.shp-wavebar) .hero-in{
