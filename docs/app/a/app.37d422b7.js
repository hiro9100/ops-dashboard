...
/* 1段ぶんの見本を、横一列の帯にして1枚の枠に入れる。
   ...
   絵を1枚ずつ差し替えるのではなく、横に送って見せる。差し替えだと
   めくるたびにいったん白くなり、見くらべている感じにならない。

   送りかたは、ブラウザ本来の横スクロールに任せる。
   はじめは帯ぜんぶを transform で動かしていたが、それだと帯が
   まるごと1枚の面になる。顔は39種あるので、横1万2千画素の面。
   iPhone は1画素を3×3の点で持つので、これだけで約160MBを取り、
   タブごと落ちた（Safari が「問題が繰り返し発生しました」を出す）。
   スクロールなら、いま見えているところだけを描くので、
   見本が何種あっても取り分は画面1枚ぶんで済む。
   指で送るのも端末まかせになり、iPhone では慣性もそのまま付く。 */
const filmCSS = (g, n) => {
...
.film{display:flex;height:100%;width:max-content}
/* 背の低い段は、枠のまん中に置く（上に寄せると下が大きく空く）。
   枠より背の高い段は、上から見せてスクロールさせる。
   safe を付けると、あふれたときに上が切れない。 */
.fp{position:relative;flex:0 0 ${Math.round(g.rw)}px;height:100%;
  scroll-snap-align:start;scroll-snap-stop:always;
  display:flex;flex-direction:column;justify-content:safe center;
  overflow-y:auto;overflow-x:hidden;background:var(--c-bg);scrollbar-width:none;
  /* 見えていない見本は、描かない。39種あっても、抱えるのは
     いま見えているぶんだけになる。大きさは外から決まっているので、
     送り先の位置がずれることはない。
     効かない端末（古い Safari）でも、ただ今までどおりになるだけ。 */
  content-visibility:auto;
  contain-intrinsic-size:${Math.round(g.rw)}px ${Math.round(g.H / g.gs)}px}
...
/* こちらから動かしているあいだの行き先。
   ...
   帯は「止まったところ」を知らせてくるが、それは少し遅れて届く。
   続けて2回送ると、1回目の到着の知らせが2回目のあとに来ることがある。
   そのまま受けると、2回目の送りが無かったことにされる（実際そうなった）。
   行き先を覚えておいて、そこに着いた知らせだけを受ける。 */
let filmWant = null;

/* いま見ている位置へ、帯を送る。枠を作り直さずに動かす。
   jump は、開いた直後など「動いて見せる必要がない」とき */
function slideFilm(jump) {
  const s = bldNow();
  const f = $('#bldFrame');
  if (!s || !f) return;
...
<body class="${esc(bodyClass())}"><div class="film" id="film">${panels}</div>
<script>${SITE_JS}<\/script><script>${FILM_JS}<\/script></body></html>`;

  /* srcdoc を差し替えるたびに load が来るので、毎回付け直す
     （once で1回だけにすると、2段目以降が反応しなくなる）。
     描き直した直後は、見ていた位置へ動かさずに置く */
  f.onload = () => slideFilm(true);
}

/* 帯からの知らせを受ける。指で送ったときの「いま何枚目か」が来る。

   受けるのは、その枠から来たものだけ。この道具は embed.html で人の
   ページに埋め込んで使えるようにしてあるので、外側のページから同じ
   合図を投げられる。いまは見本が動くだけだが、この入口に別の用が
   増えたときに、外から触れる穴として残しておきたくない。 */
addEventListener('message', (e) => {
  const f = $('#bldFrame');
  if (!f || e.source !== f.contentWindow) return;
  const d = e.data || {};
  /* 指で送って止まった位置。こちらから動かし直さない（送り合いになる） */
  if (typeof d.filmAt === 'number') {
    const s = bldNow();
    if (bldStage !== 'pick' || !s || s.off) return;
    const i = Math.max(0, Math.min(s.list.length - 1, d.filmAt));
    /* こちらから送っている最中は、行き先に着いた知らせだけを受ける。
       途中の位置を受けると、続けて押したぶんが巻き戻る */
    if (filmWant !== null) {
      if (i !== filmWant) return;
      filmWant = null;
    }
    if (i === s.i) return;
    s.i = i;
  ...
  if (d.film === 1 || d.film === -1) bldTurn(d.film);
});
