...
/* ================================================================
   順を追って組む
     ...
     ① 業種をえらぶ
     ② サイトの顔をえらぶ
     ③〜 中身を1段ずつ積む（アバウト → 強み → 写真 → 料金 → …）
     ④ 組み上がった全体を見て、細部の調整へ

   ②以降は、どの段も同じ見せかたにする。
   ・まん中に「実物」を大きく1つ。並んだ小さなカードでは、それが
     自分の店に合うのかが掴めない（顔はサイトの質に直結する）。
   ・左右の矢印とスワイプで、ほかのかたちを見くらべる。
   ・要らない段は ✕ で外す。先に「何が要るか」を全部聞いていたが、
     はじめての人はそこで止まる——「うちに何が要るのか」が分からない。
     実物を見せて「これは要る／要らない」を1つずつ決めるほうが早い。
   ・つぎへで、下から上へ切り替わって次の段に移る。上に積み上がって
     いく動きが、そのままページの組み上がりに重なる。

   どの段でも、その時点でページはもう組み上がっている。途中でやめても
   完成品が手元に残る。
   ================================================================ */
...
/* 積む順。よく要るものだけを並べる。ここに無いものも、あとから足せる。
   多く並べるほど「つぎへ」を押す回数が増えるので、欲張らない。 */
const BLD_NEEDS = ['about', 'features', 'photos', 'menu', 'cta', 'contact'];

let bldStage = 'ind';      // 'ind' | 'pick' | 'done'
let bldQ = [];             // 積む段の一覧
let bldQI = 0;             // いま何段目か
const bldNeedBlock = new Map();   // 段のキー → 置いたブロックのID

const bldNow = () => bldQ[bldQI] || null;
const bldStep = () => (bldStage === 'pick' ? (bldNow() || {}).kind : bldStage);

/* 段の一覧を組み立てる。先頭は必ず「顔」 */
function buildBldQueue() {
  const byKey = (k) => SECTION_PRESETS.find((p) => p.key === k);
  bldQ = [{
    kind: 'hero', key: 'hero', label: 'サイトの顔',
    about: 'いちばん上に来る一枚。ここでサイトの印象が決まります。',
    list: HERO_PRESETS, i: 0, off: false, must: true,
  }];
  BLD_NEEDS.forEach((k) => {
    const n = NEEDS.find((x) => x.key === k);
    if (!n) return;
    const list = n.picks.map(byKey).filter(Boolean);
    if (!list.length) return;
    bldQ.push({ kind: 'need', key: n.key, label: n.label, about: n.about,
      list, i: 0, off: false, must: false });
  });
  bldQI = 0;
}
...
function bldList() {
  const s = bldNow();
  return bldStage === 'pick' && s ? s.list : [];
}

function renderBldSteps() {
  /* 段は業種のぶんも入れて8つ前後になる。名前を全部並べると帯が長くなり、
     いま何段目かが逆に読めない。大づかみな4つで出す。 */
  const phase = bldStage === 'ind' ? 0 : bldStage === 'done' ? 3 : (bldQI === 0 ? 1 : 2);
  $('#bldSteps').innerHTML = ['業種', '顔', '中身', '仕上げ']
    .map((label, i) => `<span class="${i === phase ? 'on' : (i < phase ? 'past' : '')}">${esc(label)}</span>`)
    .join('');
}
...
function renderBldStrip() {
  ...
  const s = bldNow();
  const sel = bldStage === 'pick' && s ? bldNeedBlock.get(s.key) : null;
  $('#bldStrip').innerHTML = picked
    .map((b, i) => `<span class="${b.id === sel ? 'on' : ''}"><i>${i + 1}</i>${esc(bldNames.get(b.id) || BLOCKS[b.type].label)}</span>`)
    .join('');
  /* 積み上がりの帯は、中身を積みはじめてから出す。
     顔だけのときは「1 Cover」が宙に浮いて見える */
  $('#bldStrip').hidden = !(bldStage === 'pick' && bldQI > 0) || !picked.length;
...
function renderBldHead() {
  ...
  $('#bldStage').hidden = st !== 'pick';
  $('#bldFinish').hidden = st !== 'done';

  if (st === 'pick' && s) {
    $('#bldTitle').textContent = s.label;
    $('#bldSub').textContent = s.about;
    /* 顔は外せない。無いとページが始まらない */
    $('#bldOff').hidden = !!s.must;
    $('#bldOff').classList.toggle('on', s.off);
    $('#bldOff').textContent = s.off ? 'やっぱり入れる' : 'この段はいらない';
    $('#bldNoMask').hidden = !s.off;
  ...
  $('#bldBack').hidden = st === 'ind';
  $('#bldDone').textContent = st === 'done' ? '編集をはじめる' : 'つぎへ';
  $('#bldDone').disabled = false;
}
...
/* 帯の中で、指を横に振ったときだけ、めくる合図を親へ送る。
   縦に振ったときは、その段の中身を読むためのスクロールとして残す
   （段によっては、枠より背が高いものがある）。 */
const FILM_JS = `
(function(){
  var sx=0, sy=0, dx=0, on=false;
  function send(d){ parent.postMessage({film:d}, '*'); }
  addEventListener('touchstart', function(e){
    var t=e.touches[0]; sx=t.clientX; sy=t.clientY; dx=0; on=true;
  }, {passive:true});
  addEventListener('touchmove', function(e){
    if(!on) return;
    var t=e.touches[0];
    dx = t.clientX - sx;
    if(Math.abs(dx) < Math.abs(t.clientY - sy)) on=false;   /* 縦に読んでいる */
  }, {passive:true});
  addEventListener('touchend', function(){
    if(on && Math.abs(dx) > 44) send(dx < 0 ? 1 : -1);
    on=false;
  }, {passive:true});
  /* パソコンでも、押したまま横に引けばめくれる */
  var md=false;
  addEventListener('mousedown', function(e){ md=true; sx=e.clientX; sy=e.clientY; dx=0; });
  addEventListener('mousemove', function(e){ if(md) dx = e.clientX - sx; });
  addEventListener('mouseup', function(e){
    if(md && Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(e.clientY - sy)) send(dx < 0 ? 1 : -1);
    md=false;
  });
})();
`;

/* 1段ぶんの見本を、横一列の帯にして1枚の枠に入れる。

   絵を1枚ずつ差し替えるのではなく、帯を横にずらして見せる。差し替えだと
   めくるたびにいったん白くなり、見くらべている感じにならない。
   帯なら、ずらすだけなので指の動きにそのまま付いてくる。 */
const filmCSS = (g, n) => {
  const u = (px) => Math.round(px / g.gs);
  return `
html,body{margin:0;height:100%;overflow:hidden;background:#0d1016}
.film{display:flex;height:100%;width:${n * 100}%;
  transition:transform .34s cubic-bezier(.2,.8,.3,1)}
/* 背の低い段は、枠のまん中に置く（上に寄せると下が大きく空く）。
   枠より背の高い段は、上から見せてスクロールさせる。
   safe を付けると、あふれたときに上が切れない。 */
.fp{position:relative;flex:0 0 ${100 / n}%;height:100%;
  display:flex;flex-direction:column;justify-content:safe center;
  overflow-y:auto;overflow-x:hidden;background:var(--c-bg);scrollbar-width:none}
/* 名前の札は絵の上に浮かせるので、この決めごとから外す */
.fp > :not(.fp-name){flex:none;width:100%}
.fp::-webkit-scrollbar{display:none}
/* かたちの名前は、絵の上に小さく重ねる。下に置くと、そのぶん実物が小さくなる */
.fp-name{
  position:absolute;z-index:9;left:${u(12)}px;top:${u(12)}px;
  background:rgba(12,14,18,.72);color:#fff;border-radius:${u(999)}px;
  font-size:${u(11)}px;font-weight:700;letter-spacing:.04em;
  padding:${u(5)}px ${u(12)}px;pointer-events:none;
  font-family:"Helvetica Neue",Arial,"Hiragino Sans",Meiryo,sans-serif
}
...
/* いま見ている位置へ、帯をずらす。枠を作り直さずに動かす */
function slideFilm() {
  const s = bldNow();
  const f = $('#bldFrame');
  if (!s || !f || !f.contentDocument) return;
  const film = f.contentDocument.getElementById('film');
  if (film) film.style.transform = `translateX(-${(s.i * 100) / s.list.length}%)`;
}

function renderBldGallery() {
  if (bldStage !== 'pick') return;
  const s = bldNow();
  if (!s) return;
  ...
  const f = $('#bldFrame');
  const list = s.list;
  /* 実物を主役にする段なので、枠は大きく取る。
     カードを並べていたときのような「枚数に合わせた高さ」は要らない。 */
  const wrap = f.parentElement;
  wrap.style.flex = '0 0 auto';
  wrap.style.minHeight = '0';
  wrap.style.height = `${Math.round(Math.min(window.innerHeight * 0.56, 560))}px`;
  const g = filmGeom(f);
  sizeFilmFrame(f, g);

  const panels = list.map((p) => `<div class="fp" data-key="${esc(p.key)}">
      <span class="fp-name">${esc(p.label)}</span>
      ${presetSample(p)}
    </div>`).join('');

  /* 同じ段を描き直すだけのときは、見ている位置を保つ */
  const view = `${s.key}|${list.length}`;
  const same = view === galView;
  galView = view;

  f.srcdoc = `<!DOCTYPE html><html lang="ja"><head><meta charset="utf-8">
<style>${themeCSS(state.theme)}\n${SITE_CSS}\n${filmCSS(g, list.length)}\n${shapeMaskCSS(galShapes(list))}</style></head>
<body class="${esc(bodyClass())}"><div class="film" id="film"${
  s.i ? ` style="transform:translateX(-${(s.i * 100) / list.length}%)"` : ''}>${panels}</div>
<script>${SITE_JS}<\/script><script>${FILM_JS}<\/script></body></html>`;

  /* srcdoc を差し替えるたびに load が来るので、毎回付け直す
     （once で1回だけにすると、2段目以降が反応しなくなる） */
  f.onload = () => { if (!same) slideFilm(); };
}

/* 帯からの「めくって」を受ける。枠の外の矢印と同じところへ通す。

   受けるのは、その枠から来たものだけ。この道具は embed.html で人の
   ページに埋め込んで使えるようにしてあるので、外側のページから同じ
   合図を投げられる。いまは見本がめくれるだけだが、この入口に別の用が
   増えたときに、外から触れる穴として残しておきたくない。 */
addEventListener('message', (e) => {
  const f = $('#bldFrame');
  if (!f || e.source !== f.contentWindow) return;
  const d = e.data && e.data.film;
  if (d === 1 || d === -1) bldTurn(d);
});
...
/* 組み上がった全体。中でスクロールして、ぜんぶ見られる。
   細部を直しはじめる前に、いちど通しで見えるようにしておく。 */
function renderBldWhole() {
  if (bldStage !== 'done') return;
  const f = $('#bldWhole');
  if (!f) return;
  const g = galGeom(f);
  sizeGalFrame(f, g);
  f.style.height = `${Math.round(g.H / g.gs)}px`;
  f.srcdoc = pageHTML(page(), false);
}
...
/* いま見ているかたちを、ページに反映する。
   ...
   段のキーごとに「どのブロックを置いたか」を覚えておき、選び直したら
   その場で入れ替える（位置は動かさない）。外した段は消す。
   これを段が変わるたびに通すので、どこでやめても完成品が残る。 */
function commitStep(s) {
  if (!s) return;
  const oldId = bldNeedBlock.get(s.key);
  const at = page().blocks.findIndex((b) => b.id === oldId);

  /* 外した段。置いてあれば取り除く */
  if (s.off) {
    if (at >= 0) {
      page().blocks.splice(at, 1);
      bldNames.delete(oldId);
    }
    bldNeedBlock.delete(s.key);
    return;
  }

  const p = s.list[s.i];
  if (!p) return;
  /* 同じかたちのままなら、作り直さない（写真や打った字が消える） */
  if (at >= 0 && bldNames.get(oldId) === p.label) return;

  const nb = makeBlock(p.type, p.props);
  fillFromBiz(nb);
  bldNames.set(nb.id, p.label);
  bldNeedBlock.set(s.key, nb.id);

  if (at >= 0) {
    page().blocks.splice(at, 1, nb);
    bldNames.delete(oldId);
  } else if (s.kind === 'hero') {
    /* 顔は1ページに1つ。前のものがあれば入れ替える */
    const old = page().blocks.find((b) => b.type === 'hero' || b.type === 'collage');
    if (old) {
      const oi = page().blocks.indexOf(old);
      page().blocks.splice(oi, 1, nb);
      bldNames.delete(old.id);
    } else {
      page().blocks.splice(1, 0, nb);
    }
  } else {
    /* 中身は、フッターの手前に順に積む。積む順＝段の順になる */
    const fi = page().blocks.findIndex((b) => b.type === 'footer');
...
/* 段の切り替え。下から上へ、シュッと。
   ...
   いったん上へ抜けさせてから、中身を入れ替えて、下から上げる。
   入れ替えを先にやると、動く前に次の絵が見えてしまい、
   「上に積み上がった」ようには見えない。 */
function bldSlide(fn) {
  const box = $('#bldStage');
  const reduce = matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (!box || box.hidden || reduce) { fn(); return; }
  ...
  setTimeout(() => {
    ...
    /* 中身が入れ替わったあとで、下から上げる */
    if (!$('#bldStage').hidden) {
      $('#bldStage').classList.add('in');
      setTimeout(() => $('#bldStage').classList.remove('in'), 320);
    }
  }, 170);
}
...
function bldUndo() {
  if (bldStage === 'done') {
    bldSlide(() => { bldStage = 'pick'; bldQI = bldQ.length - 1; refreshBld(); });
    return;
  }
  if (bldStage === 'pick' && bldQI > 0) {
    bldSlide(() => { bldQI -= 1; refreshBld(); });
    return;
  }
  if (bldStage === 'pick') {
    bldSlide(() => { bldStage = 'ind'; refreshBld(); });
  }
}
...
$('#bldDone').addEventListener('click', () => bldNext());
$('#bldPrev').addEventListener('click', () => bldTurn(-1));
$('#bldNext').addEventListener('click', () => bldTurn(1));
/* この段は要らない、を切り替える。押しても消さずに覆うだけにして、
   すぐ戻せるようにする（消えると、戻し方が分からなくなる） */
$('#bldOff').addEventListener('click', () => {
  const s = bldNow();
  if (!s || s.must) return;
  s.off = !s.off;
  commitStep(s);
...
/* 左右のキーでもめくれる（枠の外にいるとき用。中は帯側で受ける） */
addEventListener('keydown', (e) => {
  if ($('#buildModal').hidden || bldStage !== 'pick') return;
  if (e.target.matches('input,textarea')) return;
  if (e.key === 'ArrowRight') bldTurn(1);
  if (e.key === 'ArrowLeft') bldTurn(-1);
});

function bldNext() {
  if (bldStage === 'ind') {
    bldSlide(() => { bldStage = 'pick'; bldQI = 0; refreshBld(); });
    return;
  }
  if (bldStage === 'pick') {
    /* いま見ているかたちを、ページに入れてから次へ */
    commitStep(bldNow());
    selected = page().blocks[1]?.id || page().blocks[0]?.id;
    selectedEl = null;
    refresh();
    if (bldQI < bldQ.length - 1) {
      bldSlide(() => { bldQI += 1; refreshBld(); });
    } else {
      bldSlide(() => { bldStage = 'done'; refreshBld(); });
    }
    return;
  }
  finishBuild();
